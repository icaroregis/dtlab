...
export default function ButtonTableActionPlan({ route }) {
  ...
  const { id, pageExct, perPage } = route.params;
  const convertedId = JSON.stringify(id);
  const image = require('../../assets/plano-de-fundo.webp');

  const listActionPlan = useFetch(`/contract-item-actions/${convertedId}`);
  const listEquip = useFetch('/contract-items?all=true');

  useEffect(() => {
    setData({
      ...data,
      solution: listActionPlan.data?.model.solution,
      description: listActionPlan.data?.model.description,
      id_contract_item: listActionPlan.data?.model.id_contract_item,
      started_at: listActionPlan.data?.model.started_at,
    });
  }, [listActionPlan.data]);
  ...
  async function handleFinish() {
    await axios
      .put(
        urlBase + group + `/contract-item-actions/${convertedId}`,
        data,
        config,
      )
      .then((r) => {
        setData({
          description: r.data.model.description,
          solution: r.data.model.solution,
          id_contract_item: r.data.model.id_contract_item,
          started_at: r.data.model.started_at,
        });
      ...
      <Card style={style.card} transparent>
        ...
        <View style={style.containerBodyIcon}>
          <MaterialCommunityIcons
            ...
            color={cores.preto}
          />
          <Text style={style.containerText}>Atualizar Plano de Ação</Text>
        </View>
        <Text style={style.containerSubHeader}>
          Gestão de Atualização de Plano de Ação
        </Text>
        {/* {icone} */}
        ...
        <View style={style.informationContainer}>
          ...
          <View style={style.ColTwo}>
            ...
            >
              <Text>{moment(data.started_at).format('DD/MM/YYYY')}</Text>
            </TouchableOpacity>
            {show && (
              <DateTimePicker
